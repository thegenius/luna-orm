...
#[derive(TemplateRecord, Clone, Debug)]
#[TemplateSql = "select * from ${name} #{age} 'hello' "]
...
#[derive(TemplateRecord, Clone, Debug)]
#[TemplateSql = "select * from ${name} #{age} \"hello ${name}\" "]
...
pub async fn template_macro_spec() -> taitan_orm::Result<()> {
    let template = TestTemplate1 { name: Cow::Borrowed("wang") };
    let sql = template.get_sql(None);
    assert_eq!(sql, "select * from wang");

    let template = TestTemplate2 { name: String::from("wang") };
    let sql = template.get_sql(None);
    assert_eq!(sql, "select * from ?");

    let template = TestTemplate3 { name: Cow::Borrowed("wang"), age: 23 };
    let sql = template.get_sql(None);
    assert_eq!(sql, "select * from wang ?");


    let template = TestTemplate4 { name: Cow::Borrowed("wang"), age: 23 };
    let sql = template.get_sql(None);
    assert_eq!(sql, "select * from wang ? 'hello'");

    let template = TestTemplate5 { name: Cow::Borrowed("wang"), age: 23 };
    let sql = template.get_sql(None);
