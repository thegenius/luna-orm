...
// use luna_orm_trait::*;
//
// use sqlx::{Database, IntoArguments, Pool, Row};
// use crate::prelude::AnyRow;
//
// pub trait SqlExecutor {
//
//     type DB: Database;
//
//     fn get_pool(&self) -> LunaOrmResult<&Pool<Self::DB>> {
...
//     {
//         let query = sqlx::query(stmt).try_map(|row: dyn Row<Database = Self::DB>| SE::from_any_row(row));
...
//     }
//
//     async fn execute_with_db<'q, DB: Database, A: IntoArguments<'q, DB>>(&mut self, stmt: &str, args: A) -> LunaOrmResult<DB::QueryResult> {
//         Ok(sqlx::query_with(stmt, args)
//             .execute(self.get_pool()?)
//             .await?
//         )
