...
use luna_orm_trait::{OrderBy, Pagination, Selection};
use luna_orm_trait::schema_trait::{EntityNew, LocationNew, MutationNew, PrimaryNew};
...
impl SqlGenerator for MySqlGenerator {
    fn get_upsert_sql(&self, entity: &dyn EntityNew) -> String {
    ...
    fn get_create_sql(&self, entity: &dyn EntityNew) -> String {
        let table_name = entity.get_table_name();
        let field_names = entity.get_insert_fields();
        let fields = wrap_fields(&field_names, self.get_wrap_char());
        let marks = generate_question_mark_list(&field_names);
...
pub trait SqlGenerator {
    ...
    fn get_select_sql(&self, selection: &dyn Selection, primary: &dyn PrimaryNew) -> String {
    ...
    fn get_search_count_sql(&self, location: &dyn LocationNew) -> String {
    ...
    fn get_insert_sql(&self, entity: &dyn EntityNew) -> String {
        let table_name = entity.get_table_name();
        let field_names = entity.get_insert_fields();
        let fields = wrap_fields(&field_names, self.get_wrap_char());
        let marks = generate_question_mark_list(&field_names);
    ...
    fn get_update_sql(&self, mutation: &dyn MutationNew, primary: &dyn PrimaryNew) -> String {
        let table_name = primary.get_table_name();
    ...
    fn get_change_sql(&self, mutation: &dyn MutationNew, location: &dyn LocationNew) -> String {
        let table_name = location.get_table_name();
    ...
    fn get_delete_sql(&self, primary: &dyn PrimaryNew) -> String {
        let table_name = primary.get_table_name();
    ...
    fn get_purify_sql(&self, location: &dyn LocationNew) -> String {
        let table_name = location.get_table_name();
        let where_clause = location.get_where_clause(self.get_wrap_char(), self.get_place_holder());
        let delete_sql = format!(
            "DELETE FROM {}{}{} WHERE {}",
